...
@MainActor
@main
struct SourceLevelMeterPollingControllerTests {
    static func main() async {
        testStartConnectsOnceAndStopCancels()
        testTickRefreshesOnlyWhileStarted()
        print("source level meter polling controller tests passed")
    }

    private static func testStartConnectsOnceAndStopCancels() {
    ...
    private static func testTickRefreshesOnlyWhileStarted() {
        let scheduler = FakeMeterScheduler()
        var refreshCount = 0
        let controller = SourceLevelMeterPollingController(
