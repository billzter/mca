...
struct SourceLevelMeterModelTests {
    static func main() async {
        ...
        testRefreshReportsRawSnapshotBeforeDecay()
        testAutoVerifierRequiresRecorderActivity()
        testAutoVerifierRequiresRawSystemAudioAboveThreshold()
        testAutoVerifierFiresOnceAfterConsecutiveProofTicks()
        testAutoVerifierResetsWhenSignalDrops()
    ...
    private static func testRefreshReportsRawSnapshotBeforeDecay() {
        let controller = FakeMeterLiveMixerController()
    ...
    private static func testAutoVerifierRequiresRecorderActivity() {
        var proofCount = 0
    ...
    private static func testAutoVerifierRequiresRawSystemAudioAboveThreshold() {
        var proofCount = 0
        let verifier = SystemAudioAutoVerifier(requiredConsecutiveTicks: 2) {
            proofCount += 1
        }
    ...
    private static func testAutoVerifierFiresOnceAfterConsecutiveProofTicks() {
        var proofCount = 0
        let verifier = SystemAudioAutoVerifier(requiredConsecutiveTicks: 2) {
            proofCount += 1
        }
    ...
    private static func testAutoVerifierResetsWhenSignalDrops() {
        var proofCount = 0
        let verifier = SystemAudioAutoVerifier(requiredConsecutiveTicks: 2) {
            proofCount += 1
        }
